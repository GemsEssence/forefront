module Forefront
  class Lead < ApplicationRecord
    ...
    scope :overdue, -> { where("due_at < ? AND status NOT IN (?)", Time.current, ['won', 'lost']) }
    scope :due_soon, -> { where("due_at BETWEEN ? AND ? AND status NOT IN (?)", Time.current, 24.hours.from_now, ['won', 'lost']) }
    ...
    def overdue?
      due_at.present? && due_at < Time.current && !won? && !lost?
    end

    def due_soon?
      due_at.present? && due_at.between?(Time.current, 24.hours.from_now) && !won? && !lost?
    end
